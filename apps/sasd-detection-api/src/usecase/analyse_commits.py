"""
src/usecase/analyse_commits.py
"""
...
class AnalyseCommitsUseCase(ABC):
    """
    Orchestrates fetching and analysing commit messages
    """
    @abstractmethod
...
class AnalyseCommitsUseCaseImpl(AnalyseCommitsUseCase):
    ...
    def __call__(self, repo_owner: str, repo_name: str) -> list[SasdFinding]:
        commits = self._artefacts.fetch_commits(repo_owner, repo_name)
        return self._analysis.analyse_commits(commits)
